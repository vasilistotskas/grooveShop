...
import EntityBaseTransformable from '@/store/common/EntityBaseTransformable'
import EntityUpdatableItemFieldType from '@/store/common/EntityUpdatableItemFieldType'
import EntityUpdatableInputTextOption from '@/store/common/EntityUpdatableInputTextOption'
...
export default abstract class EntityUpdatableTransformable<DEST, SOURCE extends EntityBase>
	...
	setupInputFieldText(
		...
		if (!isUndefined(fieldValue) && isString(fieldValue))
	...
	setupInputFieldCheckbox(
		...
		const fieldValue = get(from, sourceField)
		if (!isUndefined(fieldValue) && isBoolean(fieldValue))
			finalValue = fieldValue
