...
import ProductFavouriteModel from '@/store/product/favourite/ProductFavouriteModel'

const toast = useToast()

@Module({
	name: 'Module/ProductFavouriteModule',
	namespaced: true,
	stateFactory: true
})
export default class ProductFavouriteModule
	...
	get getstoreIsCurrentProductInFavourites(): boolean {
	...
	async toggleFavourite(product: ProductFavouriteModel): Promise<any> {
		...
		if (IsAuthenticated) {
			try {
				if (!this.getstoreIsCurrentProductInFavourites) {
