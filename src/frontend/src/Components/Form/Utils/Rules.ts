...
export const min = (min: number) => (msg: string) => (x: Array<Record<string, unknown>>) =>
  x.length >= min || msg
export const max = (max: number) => (msg: string) => (x: Array<Record<string, unknown>>) =>
  x.length <= max || msg
export const exactly = (min: number) => (msg: string) => (x: Array<Record<string, unknown>>) =>
  x.length === min || msg
export const minMax =
